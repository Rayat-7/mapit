...
export default function MapCanvas() {
  ...
  useEffect(() => {
    ...
    try {
        ...
        map.current.on('click', (e) => {
          ...
          // Only handle map clicks if NOT dragging
          if (isDragging) return;

          // If clicking map (not item), deselect
          ...
          if (state.drawingMode === 'route') {
            ...
            // Auto-switch to select mode and select the new label to open editor
            state.setDrawingMode('idle');
  ...
  useEffect(() => {
      ...
      // Add listeners to window to catch drag outside map or release anywhere
  ...
  const handleItemMouseDown = (e: React.MouseEvent, id: string, type: 'marker' | 'label') => {
    ...
    if (drawingMode !== 'idle') return; // Only drag in idle mode
    
    // Save history checkpoint BEFORE dragging starts
    saveCheckpoint();
  ...
  return (
    <div id="map-export-container" className="relative w-full h-screen" style={{ height: '100vh', width: '100%' }}>
      <div id="map-layer-container" ref={mapContainer} className="absolute inset-0 z-0 bg-gray-200" style={{ height: '100%', width: '100%' }} />
